...
int_unique <- list(dates_change = c(phase1, phase3, unlock1), 
                   R0_change = c(0.5, 0.7, 0.8)) 
...
plotting_dates <- data.frame(date = as.Date(c(phase1, phase3, unlock1)),
...
p <- projections(r = out, time_period = 5)
...
projection_plotting(
  ...
  date_0 = max(df$date), 
  x_var = "date") +
  ggtitle("Projection for hospital bed demand") +
  ylab("No. of beds") +
  xlab("Date") +
  ...
  geom_vline(data = plotting_dates,                # Add lockdown lines
  ...
  geom_text(data = plotting_dates,                   # Annotate lockdown lines
            mapping = aes(x = date, 
                          y = hosp_bed * 0.9, 
                          label = event), 
            size = 3.5, angle = 90, vjust = -0.5, hjust = .9, 
            color = 'darkgrey', alpha = 0.8, 
            fontface = 'bold') +
  geom_vline(data = today,                      # Plot "today" line
             mapping = aes(xintercept = date),
             color = 'darkmagenta',
             alpha = 0.3,
             size = 1) +
  geom_text(data = today,                          # Annotate today line
            mapping = aes(x = date,
                          y = 0,
                          label = event),
            size = 3.5, angle = 90, vjust = -0.5, hjust = 0,
...
ggsave("./playground/skye/plots/hosp_demand.png")
...
projection_plotting(
  ...
  date_0 = max(df$date), 
  x_var = "date") +
  ggtitle("Projection for ICU bed demand") +
  ylab("No. of beds") +
  xlab("Date") +
...
ggsave("./playground/skye/plots/ICU_demand.png")
